...
cmd_upDown_left = [0x55, 0x55, 0x05, 0x06, 0x00, 0x04, 0x00]
cmd_upDown_right = [0x55, 0x55, 0x05, 0x06, 0x00, 0x03, 0x00]
...
class PyQtMainEntry(QMainWindow, Ui_MainWindow):
    ...
    def camera_status(self):
        ...
        if self.is_camera_opened:
            self.pushButton_cameraClick.setText("关闭摄像头")
            self._timer.start()
        else:
            self.pushButton_cameraClick.setText("打开摄像头")
            self._timer.stop()
    ...
    def transform_to_highLeg(self):
        ...
        if self.body_status == no_rect:  # rect -> highLeg
            ...
            self.body_status = no_highLeg
            pass
        elif self.body_status == no_hexagon:  # hexagon -> highLeg
            self.body_status = no_highLeg
            serialHandle.write(cmd_rect_init)
            print(serialHandle.read(7))
        elif self.body_status == no_crab:  # crab -> highLeg
            self.body_status = no_highLeg
            serialHandle.write(cmd_rect_init)
            print(serialHandle.read(7))
        elif self.body_status == no_upDown:  # upDown -> highLeg
            self.body_status = no_highLeg
            serialHandle.write(cmd_transform_upDownToRect)
            print(serialHandle.read(7))
    ...
    def transform_to_upDown(self):
        ...
        elif self.body_status == no_highLeg:  # highLeg -> upDown
            self.body_status = no_upDown
            serialHandle.write(cmd_rect_init)
        elif self.body_status == no_hexagon:  # hexagon -> upDown
            self.body_status = no_upDown
            serialHandle.write(cmd_rect_init)
        elif self.body_status == no_crab:  # crab -> upDown
            self.body_status = no_upDown
            serialHandle.write(cmd_rect_init)
